...
Note that the function `as.data.table` creates a copy of the data frame being converted. However, if working with a large data frame it is helpful to avoid this by using `setDT`, which coerces lists and data frames to `data.table` by reference:
